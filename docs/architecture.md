...
Protected run `31621725290` found 32 fixed `HIGH` vulnerabilities in the upstream Neko 3.1.5 image: 18 in Debian packages and 14 in `/usr/bin/neko`. That image cannot pass the repository's fixed-vulnerability gate. The owned image build publishes `linux/amd64` and `linux/arm64` variants with SBOM and provenance attestations; publish run `31625537828` produced the public index now pinned by the runtime. The unchanged Trivy 0.73.0 gate reported zero fixed `HIGH` or `CRITICAL` findings for both exact child images. The amd64 child passed a live `/health` request. The arm64 child received executable-format and embedded-provenance inspection but no native boot test.
