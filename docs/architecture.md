...
The Neko Chromium image is selected by a digest-pinned `NEKO_IMAGE` value mirrored in the environment example, Compose fallback, and runtime test. The live persistence harness reads that value from the environment example. The control image is built locally from digest-pinned Go and Alpine base images. Repository checks reject inconsistent Neko pins, a hardcoded acceptance-harness digest, mutable Docker bases, and GitHub Actions without full commit SHAs.
