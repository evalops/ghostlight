...
RUN_ID="${GHOSTLIGHT_NATIVE_PERFORMANCE_RUN_ID:-}"
...
[[ -n "$VIEWER_URL" && -n "$PASSWORD" && -n "$EXPECTED_CODEC" && -n "$SOURCE_SHA" && -n "$RUN_ID" ]] || { printf 'native performance environment is incomplete\n' >&2; exit 1; }
...
(
  while kill -0 "$APP_PID" >/dev/null 2>&1; do
    ...
    ps -axo pid=,ppid=,%cpu=,rss=,command= \
      | awk -v captured_at="$captured_at" -v app_pid="$APP_PID" -f "$ROOT_DIR/tools/native-performance-processes.awk" \
      >>"$CPU_SAMPLES"
    sleep 1
  done
) &
