...
On `2026-08-12`, the exact-head improvement Linux lane built the stack, required healthy viewer and control services, recreated both containers, and restored two synthetic tabs with their cookie and local-storage marker. Its test-only override shares the viewer network namespace with control because the nested host filters sibling-container bridge traffic. A Computer Use run also reached `Viewer loaded` in the packaged app, terminated it, and relaunched from the saved control URL without another Connect action; this proves WebKit navigation, not authenticated Neko or decoded WebRTC media. The VP8 performance lane completed with 251 decoded frames, zero dropped frames, 1.17 Mbps received bitrate, a 72.43 ms browser-dispatch-to-presented-frame sample, viewer CPU samples from 3.62% to 96.01%, and viewer memory samples from 272.4 MiB to 376 MiB.
