...
The Chromium profile remains the authoritative website-state store. The control database indexes product state and the latest browser snapshot. Cookies and page contents remain in the Chromium profile. Removing the profile makes its prior website state unavailable; removing the control-state volume removes the product catalog and lease epochs.
