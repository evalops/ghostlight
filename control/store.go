...
func (s *sqliteStore) initialize(ctx context.Context) error {
	...
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS peripheral_grants (id TEXT PRIMARY KEY, workspace_id TEXT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE, session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE, client_id TEXT NOT NULL, idempotency_key TEXT NOT NULL, request_hash TEXT NOT NULL, capability TEXT NOT NULL, direction TEXT NOT NULL, origin TEXT NOT NULL, expires_at TEXT NOT NULL, created_at TEXT NOT NULL, revoked_at TEXT, UNIQUE(client_id,idempotency_key))`,
		`CREATE INDEX IF NOT EXISTS peripheral_grants_lookup ON peripheral_grants(workspace_id,session_id,client_id,capability,direction,origin)`,
		`CREATE TABLE IF NOT EXISTS peripheral_audit_events (id TEXT PRIMARY KEY, grant_id TEXT REFERENCES peripheral_grants(id) ON DELETE SET NULL, workspace_id TEXT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE, session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE, client_id TEXT NOT NULL, capability TEXT NOT NULL, direction TEXT NOT NULL, origin TEXT NOT NULL, action TEXT NOT NULL, outcome TEXT NOT NULL, occurred_at TEXT NOT NULL)`,
		`CREATE INDEX IF NOT EXISTS peripheral_audit_recent ON peripheral_audit_events(workspace_id,occurred_at DESC,id DESC)`,
	}
